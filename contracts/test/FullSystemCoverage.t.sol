...
contract MockFeedCoverage {
    ...
    constructor(int256 price, uint8 dec) { _price = price; _decimals = dec; }
...
contract FullSystemCoverageTest is Test {
    ...
    function test_ProtocolTokenomicsEngine_AllMath() public view {
        ...
        ProtocolTokenomicsEngine.RedemptionCalculation memory red = engine.calculateRedemption(1000 * 10**18, 1000 * 10**18, 1000 * 10**18, 6);
        assertGt(red.assetsReceivedTokens, 0);

        ProtocolTokenomicsEngine.BondDiscountCalculation memory bond = engine.calculateBondDiscount(1000 * 10**6, 3, 10000 * 10**18);
        ...
        ProtocolTokenomicsEngine.LoanCalculation memory loan = engine.calculateLoanTerms(1000 * 10**18, 500 * 10**18, 1000, 30);
        assertTrue(loan.isLtvValid);

        ProtocolTokenomicsEngine.HealthFactorResult memory hf = engine.calculateHealthFactor(1000 * 10**18, 500 * 10**18, false);
