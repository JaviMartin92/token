          ...
          try {
            await (window as any).ethereum.request({
              ...
              params: [{
                ...
                rpcUrls: [window.location.origin + '/rpc', 'http://127.0.0.1:8545'],
              }],
            });
          } catch (addErr) {
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
        <div className="bg-gray-900 border border-red-500/30 p-8 rounded-xl max-w-md w-full text-center">
          ...
          <h2 className="text-2xl font-bold text-white mb-2">Red No Compatible</h2>
          <p className="text-gray-400 mb-6">
            Por favor cambia a la red Anvil Localhost para operar en Alpha Centauri.
          </p>
          <div className="flex flex-col gap-3">
            {chains.map((supportedChain) => (
              <button
                key={supportedChain.id}
                onClick={() => handleSwitch(supportedChain.id)}
                className="bg-blue-600 hover:bg-blue-500 text-white py-3 px-4 rounded-lg font-medium transition-colors"
              >
                Cambiar a {supportedChain.name}
              </button>
            ))}
          </div>
        </div>
