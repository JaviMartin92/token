...
async function run() {
  await userClient.writeContract({ address: contracts.USDC, abi: ERC20_ABI, functionName: 'approve', args: [contracts.TREASURY, parseEther('1000')] });
  const tx = await userClient.writeContract({ address: contracts.TREASURY, abi: TREASURY_ABI, functionName: 'deposit', args: [parseEther('1000')] });
  ...
  const opExBal = await publicClient.readContract({ address: contracts.TREASURY, abi: ERC20_ABI, functionName: 'balanceOf', args: [opExAddr] });
  console.log('OpEx Vault ALPHA Balance:', formatEther(opExBal));
}
